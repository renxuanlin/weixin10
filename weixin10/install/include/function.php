...
function install_sql()
{
	...
	//检测SQL安装文件
	$sql_file = ROOT_PATH.'./install/we10.sql';
	...
	//插入管理员数据
	$adminSql = 'insert into `'.$db_pre.'user` (`username`,`password`,`create_time`) values ("'.$admin_user.'","'.md5($admin_pwd).'","'.date('Y-m-d H:i:s').'")';
...
//sql导入完成	
	...
	//写入配置文件
	$configDefFile = ROOT_PATH.'./config/db.php';
	$updateData  = '<?php
						return [
	...
	$is_successs = file_put_contents($configDefFile,$updateData);
	if(!$is_successs)
	{
		showProgress(array('isError' => true,'message' => '生成db.php页面出错','percent' => 0.9));
	}
